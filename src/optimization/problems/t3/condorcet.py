...
if __name__ == '__main__':
    ...
    # %%
    import pandas as pd
    arr = problems_algos_hvs
    labels = algos
    n_elements = len(algos)
    wr_mat = np.zeros((problems_algos_hvs.shape[0], n_elements, n_elements))
    for i, algos_hvs in enumerate(problems_algos_hvs):
        wr_mat[i, :, :] = wilcoxon_rank(algos_hvs, algos, alternative='greater').values
    p_value = 0.05
    wr_mat_test = (wr_mat <= p_value).astype(int)
    condorcet_mat = pd.DataFrame(np.sum(wr_mat_test, axis=0), index=labels, columns=labels)
